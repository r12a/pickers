...
var consonants = new Set(['𑐥', '𑐟', '𑐚', '𑐔', '𑐎', '𑐧', '𑐡', '𑐜', '𑐖', '𑐐', '𑐦', '𑐠', '𑐛', '𑐕', '𑐏', '𑐨', '𑐢', '𑐝', '𑐗', '𑐑', '𑐳', '𑐱', '𑐲', '𑐴', '𑐩', '𑐣', '𑐞', '𑐘', '𑐒', '𑐪', '𑐤', '𑐙', '𑐓', '𑐰', '𑐬', '𑐭', '𑐮', '𑐯', '𑐫'])
var indvowels = new Set(['𑐂', '𑐃', '𑐄', '𑐅', '𑐊', '𑐌', '𑐀', '𑐁', '𑐋', '𑐍', '𑐆', '𑐈', '𑐇', '𑐉', '𑑄', '𑑅', '𑑃', ' '])
var vowels = new Set(['𑐶', '𑐷', '𑐸', '𑐹', '𑐾', '𑑀', '𑐵', '𑐿', '𑑁', '𑐺', '𑐼', '𑐻', '𑐽'])

str = ' '+str.normalize('NFC')+'        '

// stop viramas being treated as combining characters
// (otherwise they're difficult to extract from cchar sequences later)
str = str.replace(/𑑂/g,'V')
...
for (let i=0; i<strArray.length-1; i++) {
	...
	else if (consonants.has(strArray[i]) && (consonants.has(strArray[i+1]) || indvowels.has(strArray[i+1])))  { withInherents += strArray[i]+'ʘ' }
    
	else if (consonants.has(strArray[i]) && strArray[i+1] === '\u{11446}' && ((consonants.has(strArray[i+2]) || indvowels.has(strArray[i+2])))) { // nukta
        withInherents += strArray[i]+strArray[i+1]+'ʘ'
        i++
        }
