var defaults = new Object;

defaults.font = 'Doulos SIL WF' // font name per usage in CSS
defaults.size = "32";  // number of pixels (just the number)
defaults.rows = "1.5"; // number representing number of times 100px for height of box
defaults.view = "alphabet";  // alphabet, fontgrid
defaults.language = 'vi'
defaults.blocklocation= '/scripts/latin/block'
defaults.uifont = 'Doulos SIL'; // font name per usage in CSS
defaults.uisize = "32";  // number of pixels (just the number)
defaults.ccbase = "\u25CC";  // default base for combining characters


