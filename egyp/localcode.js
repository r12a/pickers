...
function event_mouseoverSelector () {
    // display the title when moving across selectors
    // like event_mouseoverChar but doesn't affect colours & doesn't include descriptions
...
function searchForKeywords ( str, usage ) { 
	...
	for (o=0;o<foundList.length;o++) {
		...
		out += `<span class="c" title="${ spreadsheetRows[foundList[o]][cols.ucsName] }">${ foundList[o] }</span> `
		}
