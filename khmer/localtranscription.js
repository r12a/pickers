...
// syllable is an object

	// kill vowel after final if syllable is longer than 1 character and ends in consonant
	if (syllable.length > 1 && consonants.has(syllable[syllable.length-1])) syllable += coeng

	var outstr = ''
	var series = 1
	var labials = new Set(['\u1794', '\u1795', '\u1796', '\u1797', '\u1798'])
	var velars = new Set(['\u1780', '\u1781', '\u1782', '\u1783', '\u1784'])
	var nasals = new Set(['\u1798', '\u1793', '\u1789', '\u1784', '\u178E'])
	...
	switch (syllable.initialcons.charAt(0) ) { // Initial consonant
		...
		case '\u178e':  // nno
			...
			break
		case '\u178f':  // ta
			if (syllable.presyllable && nasals.has(syllable.finalcons[0])) outstr += 'ɗ'
			else outstr += 't'
			series = 1
			break
		case '\u1790':  // tha
		...
		case '\u1793':  // no
			...
			break
		case '\u1794':  // ba
			if (syllable.muusikoatan || syllable.initialcons.length > 1) outstr += 'p'
			else outstr += 'ɓ'
			series = 1
			break
		case '\u1795':  // pha
		...
		case '\u179f':  // sa
			...
			break
		case '\u17a0':  // ha
			if (syllable.initialcons.length == 1) outstr += 'h'
			series = 1
			break
		case '\u17a1':  // la
			outstr += 'l'
	...
	switch (syllable.initialcons.charAt(1) ) { // Initial subscript consonant
		...
		case '':  // no vowel
			if (syllable.nikahit) { 
				...
				if ( series == 1)  { vowelsound = 'ɑ'; }
				else if (syllable.finalcons && labials.has(syllable.finalcons[0])) vowelsound = 'u'
				else { vowelsound = 'uə̆'; }
				}
		...
		case '\u17b6':  // aa
			if (syllable.nikahit && syllable.finalcons == 'ង') { // nik & ng
				...
				if ( series == 1)  { vowelsound = 'a'; }
				else if (syllable.finalcons && velars.has(syllable.finalcons[0])) vowelsound = 'eə̆'
				else { vowelsound = 'oə̆'; }
				}
			else {
...
function transcribeToIPA (inputStr) {

	// inputStr should have spaces around words, and use hyphens to separate word-internal syllables
	
	inputStr = inputStr.replace(/\u200B/g, ' ') // convert zwsp to space
	inputStr = inputStr.replace(/\s/, ' ').trim() // remove leading and trailing spaces
	inputStr = inputStr.replace(/-/g, '- ') // convert hyphens to hyphen + space
	
	var syllables = inputStr.split(' ')
	
	var outstr = ''
	
	var vowels = new Set(["\u17B6", "\u17B7", "\u17B8", "\u17B9", "\u17BA", "\u17BB", "\u17BC", "\u17BD", "\u17C1", "\u17C2", "\u17C3", "\u17C4", "\u17C5", "\u17BE", "\u17BF", "\u17C0", "\u17d0"])
	var independentvowels = new Set(["\u17A5", "\u17A6", "\u17A7", "\u17A9", "\u17AA", "\u17AF", "\u17B0", "\u17B1", "\u17B2", "\u17B3", "\u17AB", "\u17AC", "\u17AD", "\u17AE"])
	var consonants = new Set(["\u1780", "\u1782", "\u1781", "\u1783", "\u1784", "\u1785", "\u1787", "\u1786", "\u1788", "\u1789", "\u178A", "\u178C", "\u178B", "\u178D", "\u178E", "\u178F", "\u1791", "\u1790", "\u1792", "\u1793", "\u1794", "\u1796", "\u1795", "\u1797", "\u1798", "\u1799", "\u17A1", "\u179B", "\u179A", "\u179F", "\u17A0", "\u179C", '\u17a2'])
	var consonants = new Set(["\u1780", "\u1782", "\u1781", "\u1783", "\u1784", "\u1785", "\u1787", "\u1786", "\u1788", "\u1789", "\u178A", "\u178C", "\u178B", "\u178D", "\u178E", "\u178F", "\u1791", "\u1790", "\u1792", "\u1793", "\u1794", "\u1796", "\u1795", "\u1797", "\u1798", "\u1799", "\u17A1", "\u179B", "\u179A", "\u179F", "\u17A0", "\u179C", '\u17a2'])
	var irregularwords = { 'ព្រហ្មណ៍':'prum', 'ព្រាហ្មណ៍':'priəm'  }
	var series = 1
	vowelHarmony = ''
		
	for (let cSyllable=0; cSyllable<syllables.length; cSyllable++) {
		var glyphs = syllables[cSyllable]  
		if (debug) console.log('>'+glyphs+'<')
		if (glyphs === '') { 
			outstr += ' '
			continue
			}
		...
		for (var cChar=0; cChar<glyphs.length; cChar++) {
			character = glyphs[cChar] //alert(character.charCodeAt(0).toString(16));
			if (consonants.has(character)) { // if consonant & first character, or not inFinal and preceded by coeng, capture initial consonant
				if ( cChar === 0  || (inFinal === false && glyphs[cChar-1] === coeng)) syllable.initialcons += character
				else { // otherwise capture a final consonant & set state to inFinal
					syllable.finalcons += character
					inFinal = true
					}
				}
			else if (vowels.has(character)) { // if vowel, capture it, and record the position
				syllable.vowel = character
				vowelposn = i
				}
			else if (independentvowels.has(character)) {  // if independent vowel, also capture as vowel and record position
				syllable.vowel = character
				vowelposn = i
				}
			else if (character === robat) { // if robat & in final, delete final, otherwise set robat to true
			...
			else console.log(i+' Unknown character'+character+' '+character.charCodeAt(0).toString(16))
			}
		
		// check that this syllable has no toandakhiat (syllable sound killer) before converting
