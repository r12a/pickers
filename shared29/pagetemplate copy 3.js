...
if (show.default) {
    out += `
    <span class="vmtab palette on" title="Use ASCII characters to type ${ show.language } from the keyboard using a customised key mapping." data-title="Default type-assist: Map keyboard to characters for easy input. Press \` to switch." data-shorttitle="Default" onmouseover="showMenuText(this.dataset.title,'#666')" onmouseout="hideMenuText()" onclick="if (this.classList.contains('on')) closeSidebarPalettes(this); else { closeSidebarPalettes(this); mapstring=makeTypeAssistMap(cols.key); setUpTypeAssist(false, mapstring, mapstring); this.classList.add('on');this.classList.remove('off');} output.className = 'typeAssistDefault'" id="showRevTransSwitch">Default</span>`
    }
if (show.latin) {
    out += `
    <span class="vmtab palette off" title="Show characters needed for IPA or other transcriptions and transliterations." data-title="Type-assist: Latin characters needed for transcriptions." data-shorttitle="Latin" onmouseover="showMenuText(this.dataset.title,'#666')" onmouseout="hideMenuText()" onclick="if (this.classList.contains('on')) closeSidebarPalettes(this); else { closeSidebarPalettes(this); setUpTypeAssist(true, latinTypeAssistMap, latinTypeAssistMap); this.classList.add('on'); this.classList.remove('off');}output.className = 'typeAssistLatin'" id="showLatinTransSwitch">Latin</span>`
    }
if (show.ipa) {
    out += `
    <span class="vmtab off palette" title="Use an IPA keyboard mapping to type ${ show.language } from the keyboard." data-title="Type assist: IPA to ${ show.language }." data-shorttitle="[...]" onmouseover="showMenuText(this.dataset.title,'#666')" onmouseout="hideMenuText()" onclick="if (this.classList.contains('on')) closeSidebarPalettes(this); else { closeSidebarPalettes(this);mapstring=makeComplexTypeAssistMap(cols.ipaLoc);setUpTypeAssist(false, mapstring, mapstring); this.classList.add('on');this.classList.remove('off');} output.className = 'typeAssistOther'">[...]</span>`
    }
if (show.transc) {
    out += `
    <span class="vmtab off palette" title="Use a  mapping from ${ show.transc } to type Arabic from the keyboard." data-title="Type assist: ${ show.transc } transcription to ${ show.language }." data-shorttitle="<..>" onmouseover="showMenuText(this.dataset.title,'#666')" onmouseout="hideMenuText()" onclick="if (this.classList.contains('on')) closeSidebarPalettes(this); else { closeSidebarPalettes(this);mapstring=makeComplexTypeAssistMap(cols.transcription);setUpTypeAssist(false, mapstring, mapstring); this.classList.add('on');this.classList.remove('off');}output.className = 'typeAssistOther'">&lt;..&gt;</span>`
    }
if (show.kbd) {
    out += `
    <span class="vmtab off palette" title="Use an ${ show.kbd } keyboard mapping to type from the keyboard." data-title="Type assist: Map keys to an ${ show.language } keyboard." data-shorttitle="⌨" onmouseover="showMenuText(this.dataset.title,'#666')" onmouseout="hideMenuText()" onclick="if (this.classList.contains('on')) closeSidebarPalettes(this); else { closeSidebarPalettes(this);setUpTypeAssist(false, makeTypeAssistMap(cols.kbd), makeTypeAssistMap(cols.kbd)); document.getElementById('keyboard').style.display='block';; this.classList.add('on');this.classList.remove('off');}output.className = 'typeAssistOther'">⌨</span>`
    }
...
for (let i=0;i<inputAids.length;i++) {
    ...
    if (inputAids[i].dataShortTitle === 'T')  inputAids[i].dataShortTitle = 'Default'
    if (inputAids[i].dataShortTitle === 'L')  inputAids[i].dataShortTitle = 'Latin'
    ...
    out += ` title="Show/hide the type-assist palette. ~ also works."`
    out += ` data-title="Show or hide the palette used for type-assist input."`
    out += ` data-shorttitle="▼"`
    out += ` onMouseOver="showMenuText(this.dataset.title,'#666')"`
    out += ` onMouseOut="hideMenuText()"`
